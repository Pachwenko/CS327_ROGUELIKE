 ...
 *
 *
 * Checks if the given destination is OK to teleport to
 *
 */
int check_bounds(pair_t destination)
{
  if (destination[dim_y] < 1 || destination[dim_y] > (DUNGEON_Y - 2))
  {
    return 0;
  }
  if (destination[dim_x] < 1 || destination[dim_x] > (DUNGEON_X - 2))
  {
    return 0;
  }
  return 1;
...
uint32_t io_teleport_pc(dungeon_t *d)
{
  ...
  while (keep_asking)
  {
    ...
    // handle inputs, try to move in the direction wanted
    switch (input)
    {
    case 'r':
    ...
    case 't':
      ...
      if (check_bounds(dest))
      {
        ...
        *d->pc.position = *dest;
        ...
        // TODO: print an error message saying selected position is out of bounds
      }
      break;
    case '8':
    case 'k':
    ...
    case '7':
    case 'y':
    case KEY_HOME: // move up and left
      io_display(d);
      dest[dim_y]--;
      dest[dim_x]--;
      if (!check_bounds(dest)) {
    ...
    case '9':
    case 'u':
    case KEY_PPAGE: // move up and right
      io_display(d);
      dest[dim_y]--;
      dest[dim_x]++;
      if (!check_bounds(dest)) {
    ...
    case '3':
    case 'n':
    case KEY_NPAGE: // move down and right
    io_display(d);
      dest[dim_y]++;
      dest[dim_x]++;
      if (!check_bounds(dest)) {
    ...
    case '1':
    case 'b':
    case KEY_END: // move down and left
      io_display(d);
      dest[dim_y]++;
      dest[dim_x]--;
      if (!check_bounds(dest)) {
